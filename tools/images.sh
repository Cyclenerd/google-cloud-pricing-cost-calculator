...
echo "NAME;DESCRIPTION;DISK_SIZE_GB;PROJECT;FAMILY;CREATION;DEPRECATED;STATUS" > "$CSV_GCLOUD_DEEPLEARNING_IMAGES" || exit 9
gcloud compute images list \
	--project ml-images \
	--filter="creationTimestamp > -P1Y" \
	--no-standard-images \
	--quiet \
	--format="csv[no-heading,separator=';'](NAME,description,diskSizeGb,PROJECT,FAMILY,creationTimestamp,DEPRECATED,STATUS)" >> "$CSV_GCLOUD_DEEPLEARNING_IMAGES" || exit 9
