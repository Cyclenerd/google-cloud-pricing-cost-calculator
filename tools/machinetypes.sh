...
echo "NAME;CPUS;SHARED_CPU;MEMORY_GB;DESCRIPTION;DEPRECATED;ZONE" > "$CSV_GCLOUD_MACHINE_TYPE_ZONE" || exit 9
gcloud compute machine-types list \
	--quiet \
	--filter="ZONE:-" \
	--format="csv[no-heading,separator=';'](NAME,CPUS,isSharedCpu,MEMORY_GB,description,DEPRECATED,ZONE)" | sort -u >> "$CSV_GCLOUD_MACHINE_TYPE_ZONE" || exit 9
